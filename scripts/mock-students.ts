 ...
 *
 * Set INSTRUCTOR_PASSWORD env var for auto-detection when auth is enabled.
 *
 * Examples:
...
/** Authenticate as instructor and return session cookie, or empty string if auth is not configured. */
async function getInstructorCookie(): Promise<string> {
  const password = process.env.INSTRUCTOR_PASSWORD;
  if (!password) return "";

  const res = await fetch(`${serverUrl}/api/instructor/login`, {
    method: "POST",
  ...
  if (!res.ok && res.status !== 204) {
    throw new Error(`Instructor login failed: ${res.status} ${res.statusText}`);
  }
  const setCookie = res.headers.get("set-cookie") ?? "";
  const match = setCookie.match(/mdq_instructor_session=([^;]+)/);
  return match ? `mdq_instructor_session=${match[1]}` : "";
}
...
async function findActiveSession(): Promise<string> {
  ...
  const res = await fetch(`${serverUrl}/api/sessions/active`, { headers });
  if (res.status === 401) {
    throw new Error("Instructor auth required. Set INSTRUCTOR_PASSWORD env var.");
  }
  if (!res.ok) {
